...
func TestRouterLookup(t *testing.T) {
	...
	handle, _, tsr, _ := router.Lookup("GET", "/nope")
	if handle != nil {
	...
	handle, params, tsr, matchingRoute := router.Lookup("GET", "/user/gopher")
	if handle == nil {
		...
		if len(matchingRoute) == 0 {
			t.Fatal("`matchingRoute` is empty")
		}
		if matchingRoute != "/user/:name" {
			t.Fatal("`matchingRoute` incorrect.")
		}
	}
	...
	handle, _, tsr, _ = router.Lookup("GET", "/user/gopher/")
	if handle != nil {
		t.Fatalf("Got handle for unregistered pattern: %v", handle)
	}
	if !tsr {
