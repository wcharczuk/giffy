...
func TestAppWritesLogs(t *testing.T) {
	...
	agent := logger.All().WithWriter(logger.NewTextWriter(buffer))
...
func TestAppNotFound(t *testing.T) {
	...
	buffer := bytes.NewBuffer(nil)
	agent := logger.All().WithWriter(logger.NewTextWriter(buffer).WithShowLabel(true).WithUseColor(false).WithShowTimestamp(false))
	app := New().WithLogger(agent)
	app.GET("/", func(r *Ctx) Result {
		return r.Raw([]byte("ok!"))
	})
